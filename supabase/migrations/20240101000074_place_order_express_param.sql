-- Persist is_express at order creation.
-- Rewrites place_order (base definition from migration 058) to accept an
-- optional p_is_express flag and store it on the order INSERT, so express intent
-- is known during bidding (e.g. so accept_order can restrict express to riders).
-- Everything else is identical to 058.
...
grant execute on function public.place_order(uuid, integer, text, double precision, double precision, text, boolean) to authenticated;
