-- Migration 20240101000009 replaced handle_order_delivered() without
-- specifying SECURITY DEFINER, silently downgrading it to SECURITY INVOKER.
-- When a customer confirms delivery the trigger fires as the customer, who
-- has no UPDATE permission on provider_products — so stock was never deducted.
-- This restores SECURITY DEFINER so the function runs as the DB owner.
...
BEGIN
  IF NEW.status = 'delivered' AND OLD.status != 'delivered' THEN
    ...
    -- 1. Deduct stock for each order item from the specific provider's inventory
    FOR item IN
